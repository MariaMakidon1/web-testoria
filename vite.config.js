...
var ON_DEMAND_CHUNKS = ['export', 'excel', 'jspdf', 'pdf', 'fflate', 'editor', 'charts'];
// Transitive dependencies of @tiptap/prosemirror that are not matched by name alone.
var EDITOR_TRANSITIVE_DEPS = [
...
var PRIMEVUE_SCOPES = ['primevue', '@primevue', '@primeuix', 'primeicons'];
    ...
    build: {
        ...
        modulePreload: {
            ...
            resolveDependencies: function (_filename, deps) {
                return deps.filter(function (dep) { return !ON_DEMAND_CHUNKS.some(function (name) {
                    return dep.includes('/' + name + '-') || dep.includes('/' + name + '.');
                }); });
            }
        },
        rollupOptions: {
            output: {
                manualChunks: function (id) {
                    ...
                    if (PRIMEVUE_SCOPES.some(function (s) { return id.includes(s); })) {
                        return 'primevue';
                    }
                    // TipTap editor (on-demand) — include all prosemirror transitive deps
                    if (id.includes('@tiptap') || id.includes('prosemirror') ||
                        EDITOR_TRANSITIVE_DEPS.some(function (dep) { return id.includes('/' + dep + '/'); })) {
                        return 'editor';
                    }
